...
public class PeerService {
              ...
              .isPresent())
            {
                return userContext()
                  .map(context -> {
                      ...
                      RestResp.set("failed to add peer and eventhub Cause by this endpoint or eventhub error");
                      return null;
                  })
                  .isPresent();
            }
        } catch (Exception e) {
            RestResp.set("failed to add peer and eventhub "+peerEventhub.toString()+" :"+e.toString());
            LOG.error("failed to add peer and eventhub {}", peerEventhub, e);
        }
        RestResp.set("This peer :"+peerEventhub.getId()+" already exists");
        return false;
    }
    ...
    private boolean reachable(String endpoint) {
        ...
        try {
            ...
            RestResp.set("failed to connect to :"+endpoint+"  "+e.getMessage());
    ...
    public boolean connectToPeer(String peerId) {
        return peerRepo
          ...
          .orElseGet(() -> {RestResp.set("this peer "+peerId+" Endpoint is  disable"); return false;});
    }
    ...
    public Optional<PeerEventhub> enrollPeer(PeerEventhub peer) {
        try {
            ...
            if(!peerOptional.isPresent()){
                RestResp.set("View certificate failure Cause by  a password error");
                return empty();
            }
            ...
            RestResp.set("failed to enroll peer"+peer.toString()+" : "+e.getMessage());
