...
public class UserService {
        ...
        try {
            return userContext().map(context -> {
                ...
                if (registered) {
                    ...
                    RestResp.set("Registered user fails, user already exists");
                }
                return null;
            });
        } catch (Exception e) {
            RestResp.set("failed to register user :"+user.toString()+" :"+e.getMessage());
    ...
    public boolean revoke(String username, int reason) {
        try {
            ...
            RestResp.set("failed to revoke user {}"+username+": "+e.getMessage());
    ...
    public Optional<UserToken> tokenForUser(final User user) {
        try {
            ...
            if(!userOptional.isPresent()){
                RestResp.set("Sorry, user name or password does not exist");
            }
            return userOptional
              .map(u -> u.enrolled()
                ? u
            ...
            RestResp.set("failed to generate token for user "+user.toString()+": "+e.getMessage());
